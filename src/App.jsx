...
function AuthBox({ user, onUserChange, onRefresh }) {
  ...
  const google = async () => { if (!supabaseEnabled) { setMessage('Supabase 연결 설정이 필요해. public/supabase-config.js의 URL과 Publishable Key를 확인해줘.'); return; } setBusy(true); setMessage('Google 로그인으로 이동 중…'); const { error } = await supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: getAuthRedirectUrl() } }); if (error) { setMessage(error.message); setBusy(false); } };
