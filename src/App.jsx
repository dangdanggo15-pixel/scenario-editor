...
function SharedPreview({project,loading}) {
  ...
  const dialogue=(x,next,bg)=><div className="shared-stage" style={bg?.dataUrl?{backgroundImage:`url(${bg.dataUrl})`,backgroundSize:'cover',backgroundPosition:'center'}:undefined}>{imgForShared(project)(x.speakerId,x.illustrationNum,x.position)&&<img key={`${x.id}-${x.bounce?'b':''}`} className={`preview-character-img ${x.position} ${x.bounce?'bounce':''}`} src={imgForShared(project)(x.speakerId,x.illustrationNum,x.position)} alt=""/>}<div className="dialogue-box"><div className="preview-speaker">{display(char(x.speakerId)?.name||'')}</div><div className="preview-text"><TypewriterText key={`${x.id}-${x.text}-${playerName}`} text={display(x.text||' ')}/></div><button className="next-arrow" onClick={next}>›</button></div></div>;
