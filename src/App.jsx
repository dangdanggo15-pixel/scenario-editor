...
import { supabase, supabaseEnabled, supabaseConfig } from './lib_supabase';
...
function AuthBox({ user, onUserChange, onRefresh }) {
  ...
  const google = async () => {
    ...
    try {
      // OAuth navigation cannot attach a custom `apikey` request header.
      // Supabase's browser OAuth URL therefore needs the publishable key in
      // the query string so the API gateway can identify the public client.
      ...
      authUrl.searchParams.set('apikey', supabaseConfig.key);
      window.location.assign(authUrl.toString());
    } catch (error) {
      setMessage(error.message || 'Google 로그인에 실패했어.');
      setBusy(false);
    }
  };
