...
function normalizeProject(raw) {
  ...
  const normalizeLocationBlocks = blocks => (Array.isArray(blocks) ? blocks : []).map(b => {
    ...
    if (b.type === 'map') return { ...b, mapId: b.mapId || null };
    return b;
  });
  const maps = Array.isArray(raw.maps) ? raw.maps.map(m => ({ id: m.id || uid('map'), name: m.name || '맵', locations: Array.isArray(m.locations) ? m.locations.map(l => ({ id: l.id || uid('loc'), name: l.name || '장소', visited: Boolean(l.visited), blocks: normalizeLocationBlocks(l.blocks) })) : [] })) : [];
  ...
  return {
    ...fallback, ...raw, id: raw.id || fallback.id, name: raw.name || '새 프로젝트', characters, backgrounds, maps, chapters,
...
function allDialogueBlocks(project) {
  const top = [...project.scenes, ...project.endings].flatMap(item => item.blocks || []);
  const loc = (project.maps || []).flatMap(m => (m.locations || []).flatMap(l => l.blocks || []));
  return [...top, ...loc];
}
function dialogueCount(project) { return allDialogueBlocks(project).filter(b => b.type === 'dialogue' && String(b.text || '').trim()).length; }
function characterCount(project) { return allDialogueBlocks(project).filter(b => b.type === 'dialogue' && String(b.text || '').trim()).reduce((n,b)=>n + Array.from(String(b.text||'')).length, 0); }
...
function App() {
  const [projects,setProjects]=useState(()=>loadLocalProjects()), [projectId,setProjectId]=useState(()=>localStorage.getItem(LAST_PROJECT_KEY)), [selectedSceneId,setSelectedSceneId]=useState(null), [selectedTab,setSelectedTab]=useState('script'), [selectedMapId,setSelectedMapId]=useState(null), [selectedLocationId,setSelectedLocationId]=useState(null);
  ...
  const cloneScene=id=>{const source=project.scenes.find(s=>s.id===id);if(!source)return;const copyBlock=b=>({...b,id:uid(b.type==='dialogue'?'line':b.type==='choice'?'choice':b.type==='background'?'bgbar':b.type==='map'?'mapbar':'block'),options:b.options?.map(o=>({...o,id:uid('opt')}))});const clone={...source,id:uid('scene'),name:`${source.name} 복제`,blocks:(source.blocks||[]).map(copyBlock)};const idx=project.scenes.findIndex(s=>s.id===id);updateProject(p=>{const scenes=[...p.scenes];scenes.splice(idx+1,0,clone);return{...p,scenes,chapters:p.chapters.map(c=>c.id===clone.chapterId?{...c,sceneIds:[...c.sceneIds,clone.id]}:c)}});setSelectedSceneId(clone.id);setSelectedTab('script')};
...
function ScriptEditor({project,scene,isEnding=false,updateBlock,deleteBlock,replaceBlocks,addDialogueAfter,addChoiceAfter,addBackgroundAfter,addMapBarAfter,handleDialogueKeyDown,dialogueRefs}) {
  ...
  const ub=(id,fn)=>updateBlock(scene.id,id,fn); const db=id=>deleteBlock(scene.id,id); const ac=(index)=>addChoiceAfter(scene.id,index);
  return <div className="script-table"><div className="table-head"><div className="speaker-head">CHARACTER</div><div>NUM.</div><div>POSITION</div><div>BOUNCE</div><div>DIALOGUE</div><div/></div><div className="block-list">{scene.blocks.map((block,index)=>block.type==='dialogue'?<div className="script-row" key={block.id}><div className="speaker-cell"><select value={block.speakerId||''} onChange={e=>{const sid=e.target.value||null;ub(block.id,b=>({...b,speakerId:sid,illustrationNum:sid?b.illustrationNum:null,position:sid?b.position:'none',bounce:sid?b.bounce:false}))}}><option value="">(독백)</option>{project.characters.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></div><div className="num-cell"><select value={block.illustrationNum??''} onChange={e=>ub(block.id,b=>({...b,illustrationNum:e.target.value?Number(e.target.value):null}))} disabled={!block.speakerId}><option value="">—</option>{(project.characters.find(c=>c.id===block.speakerId)?.illustrations||[]).map(img=><option key={img.id} value={img.num}>{img.num}</option>)}</select></div><div className="position-cell"><div className="position-buttons"><button aria-label="왼쪽" className={block.position==='left'?'selected':''} disabled={!block.speakerId} onClick={()=>ub(block.id,b=>({...b,position:b.position==='left'?'none':'left'}))}>{block.position==='left'?'●':'○'}</button><button aria-label="오른쪽" className={block.position==='right'?'selected':''} disabled={!block.speakerId} onClick={()=>ub(block.id,b=>({...b,position:b.position==='right'?'none':'right'}))}>{block.position==='right'?'●':'○'}</button></div></div><div className="bounce-cell"><input type="checkbox" checked={Boolean(block.bounce)} disabled={!block.speakerId} onChange={e=>ub(block.id,b=>({...b,bounce:e.target.checked}))}/></div><textarea ref={el=>dialogueRefs.current[block.id]=el} className="dialogue-cell" value={block.text||''} placeholder="대사를 입력하세요…" onChange={e=>ub(block.id,b=>({...b,text:normalizeDialogueText(e.target.value)}))} onKeyDown={e=>handleDialogueKeyDown(e,scene.id,block,index)} rows={1}/><div className="row-actions"><button className="action-icon" title="위로" disabled={index===0} onClick={()=>reorder(index,-1)}>↑</button><button className="action-icon" title="아래로" disabled={index===scene.blocks.length-1} onClick={()=>reorder(index,1)}>↓</button><button className="action-icon action-delete" title="삭제" onClick={()=>db(block.id)}>×</button><button className="insert-choice-btn" onClick={()=>ac(index)}>＋ 선택지</button><button className="insert-extra-btn" onClick={()=>addBackgroundAfter(scene.id,index,isEnding)}>＋ 배경</button>{!isEnding&&<button className="insert-extra-btn" onClick={()=>addMapBarAfter(scene.id,index)}>＋ 맵</button>}</div></div>:block.type==='background'?<BackgroundBar key={block.id} block={block} index={index} reorder={reorder} project={project} updateBlock={ub} deleteBlock={db}/>:block.type==='map'?<MapBar key={block.id} block={block} project={project} updateBlock={ub} deleteBlock={db} index={index} reorder={reorder}/>:<ChoiceBlock key={block.id} block={block} project={project} updateBlock={ub} deleteBlock={db}/>)}</div><div className="editor-footer-actions"><button className="add-dialogue-btn" onClick={()=>addDialogueAfter(scene.id,scene.blocks.length-1,scene.blocks.at(-1))}>＋ 대사</button><button className="add-choice-btn" onClick={()=>addChoiceAfter(scene.id,scene.blocks.length-1)}>＋ 선택지</button><button className="add-background-btn" onClick={()=>addBackgroundAfter(scene.id,scene.blocks.length-1,isEnding)}>＋ 배경</button>{!isEnding&&<button className="add-map-btn" onClick={()=>addMapBarAfter(scene.id,scene.blocks.length-1)}>＋ 맵</button>}</div></div>;
}

function BackgroundBar({block,project,updateBlock,deleteBlock,index,reorder}) { return <div className="background-bar"><span>BACKGROUND</span><select value={block.backgroundId||''} onChange={e=>updateBlock(block.id,b=>({...b,backgroundId:e.target.value||null}))}><option value="">배경 선택</option>{project.backgrounds.map(bg=><option key={bg.id} value={bg.id}>{bg.name}</option>)}</select><div className="bar-move"><button className="bar-move-btn" title="위로" disabled={index===0} onClick={()=>reorder(index,-1)}>↑</button><button className="bar-move-btn" title="아래로" disabled={index===999} onClick={()=>reorder(index,1)}>↓</button></div><button className="bar-delete" onClick={()=>deleteBlock(block.id)}>×</button></div>; }
...
function MapPanel({project,selectedMapId,setSelectedMapId,selectedLocationId,setSelectedLocationId,addMap,updateMap,deleteMap,addLocation,updateMapLocation,deleteLocation,dialogueRefs}) {
  ...
  const addLocDialogue=(index,prev=null)=>{if(!map||!loc)return;const inherited=prev?.type==='dialogue'&&prev.speakerId?{speakerId:prev.speakerId,illustrationNum:prev.illustrationNum??null,position:prev.position||'none',bounce:Boolean(prev.bounce)}:{speakerId:null,illustrationNum:null,position:'none',bounce:false};const b={id:uid('line'),type:'dialogue',...inherited,text:''};updateMapLocation(map.id,loc.id,l=>{const blocks=[...l.blocks];blocks.splice(index+1,0,b);return {...l,blocks}});requestAnimationFrame(()=>dialogueRefs.current[b.id]?.focus())};
  const addLocChoice=(index)=>{if(!map||!loc)return;const b={id:uid('choice'),type:'choice',prompt:'',reactive:false,reactiveTargetId:'',options:[{id:uid('opt'),text:'',targetId:'',responseSpeakerId:null,responseText:''},{id:uid('opt'),text:'',targetId:'',responseSpeakerId:null,responseText:''}]};updateMapLocation(map.id,loc.id,l=>{const blocks=[...l.blocks];blocks.splice(index+1,0,b);return {...l,blocks}})};
  const addLocBackground=(index)=>{if(!map||!loc)return;updateMapLocation(map.id,loc.id,l=>{const blocks=[...l.blocks];blocks.splice(index+1,0,{id:uid('bgbar'),type:'background',backgroundId:project.backgrounds[0]?.id||null});return {...l,blocks}})};
  return <section className="editor-card map-editor">
    <div className="editor-header"><div><div className="eyebrow">MAP</div><h1>맵</h1><div className="editor-hint">한 번 방문한 장소는 완료 처리되고, 모든 장소를 방문하면 다음으로 진행해.</div></div><button className="dark-btn" onClick={addMap}>＋ 맵 추가</button></div>
    <div className="map-layout">
      <aside className="map-list"><div className="map-list-title">MAPS</div>{maps.map(m=><button key={m.id} className={m.id===map?.id?'active':''} onClick={()=>{setSelectedMapId(m.id);setSelectedLocationId(m.locations?.[0]?.id||null)}}>{m.name}</button>)}{!maps.length&&<div className="empty-state">아직 맵이 없어.</div>}</aside>
      {map?<div className="map-work">
        <div className="map-name-row"><input value={map.name} onChange={e=>updateMap(map.id,{name:e.target.value})}/><button className="row-delete" onClick={()=>{deleteMap(map.id);setSelectedMapId(maps.find(x=>x.id!==map.id)?.id||null);setSelectedLocationId(null)}}>맵 삭제</button></div>
        <div className="location-list">
          <div className="section-row"><div><div className="map-list-title compact">LOCATIONS</div><b>장소 리스트</b></div><button className="mini-add" onClick={()=>addLocation(map.id)}>＋ 장소</button></div>
          <div className="location-chip-list">{map.locations.map((l,i)=><button type="button" key={l.id} className={`location-chip ${l.id===loc?.id?'active':''}`} onClick={()=>setSelectedLocationId(l.id)}><span className="location-chip-num">{String(i+1).padStart(2,'0')}</span><span className="location-chip-name">{l.name||'이름 없는 장소'}</span><span className="location-chip-check">{l.visited?'✓':''}</span></button>)}</div>
...
function PreviewOverlay({project,previewSceneId,previewBlockIndex,setPreviewBlockIndex,previewEndingId,setPreviewSceneId,setPreviewEndingId,previewReaction,setPreviewReaction,goPreviewNext,previewScene,previewBlock,selectPreviewOption,illustrationFor,close,createShare,shareBusy,shareUrl,setShareUrl}) {
  ...
  const renderDialogue=(block,next,bg)=>{const img=block?.speakerId&&block.position!=='none'?illustrationFor(block.speakerId,block.illustrationNum):null;return <div className="visualnovel-preview" style={bg?.dataUrl?{backgroundImage:`url(${bg.dataUrl})`}:undefined}>{img&&<img key={`${block.id}-${block.bounce?'b':''}`} className={`preview-character-img ${block.position} ${block.bounce?'bounce':''}`} src={img} alt=""/>}<div className="dialogue-box"><div className="preview-speaker">{display(project.characters.find(c=>c.id===block?.speakerId)?.name||'')}</div><div className="preview-text"><TypewriterText key={`${block?.id||'preview'}-${block?.text||''}-${playerName}`} text={display(block?.text||' ')}/></div><button className="next-arrow" onClick={next}>›</button></div></div>};
  ...
  let body;
  if(previewReaction)body=renderDialogue({...previewReaction,text:display(previewReaction.text)},()=>goPreviewNext(previewReaction.targetId),previewScene?getBackgroundAt(previewScene.blocks,contentIndex,project):null);
  else if(!previewSceneId){const e=project.endings.find(x=>x.id===previewEndingId),ei=nextContentIndex(e?.blocks||[],previewBlockIndex),b=e?.blocks?.[ei],bg=e?getBackgroundAt(e.blocks,ei,project):null;if(b)body=renderDialogue(b,()=>ei+1<e.blocks.length?setPreviewBlockIndex(ei+1):setPreviewBlockIndex(-1),bg);else body=<div className="preview-ending-screen"><div className="preview-ending-kicker">ENDING</div><h2>{display(e?.name||'END')}</h2><p>엔딩 대본이 끝났어.</p><button className="dark-btn" onClick={()=>{setPreviewSceneId(project.scenes[0]?.id||null);setPreviewBlockIndex(0);setPreviewEndingId(null)}}>처음부터</button></div>}
  else if(block?.type==='dialogue')body=renderDialogue(block,next,getBackgroundAt(previewScene.blocks,contentIndex,project));
  else if(block?.type==='map'){const m=project.maps?.find(x=>x.id===block.mapId);body=<MapPreview map={m} project={project} playerName={playerName} illustrationFor={illustrationFor} onComplete={()=>{const ni=nextContentIndex(previewScene.blocks,contentIndex+1);setPreviewBlockIndex(ni<previewScene.blocks.length?ni:0)}}/>;}
  else if(block?.type==='choice')body=<div className="choice-preview"><div className="choice-preview-prompt">{display(block.prompt||'선택하세요.')}</div>{(block.options||[]).map(o=><button key={o.id} className="preview-choice" onClick={()=>selectPreviewOption(block,o)}>{display(o.text||'선택지')}</button>)}</div>;
  else body=<div className="preview-empty">더 진행할 내용이 없어.</div>;
  return <div className="preview-overlay"><div className="preview-window"><div className="preview-bar"><div><span className="preview-live-dot"/> PREVIEW</div><div className="preview-bar-actions"><button className="preview-share-btn" onClick={createShare} disabled={shareBusy}>{shareBusy?'링크 생성 중…':'🔗 미리보기 링크'}</button><button onClick={close}>닫기 ×</button></div></div>{shareUrl&&<div className="share-modal"><b>미리보기 링크가 생성되었어!</b><input value={shareUrl} readOnly onFocus={e=>e.target.select()}/><div><button className="ghost-btn" onClick={()=>navigator.clipboard?.writeText(shareUrl)}>복사</button><a className="dark-btn" href={shareUrl} target="_blank" rel="noreferrer">열기</a><button className="row-delete" onClick={()=>setShareUrl('')}>×</button></div></div>}<div className="preview-stage">{!started?<div className="player-name-screen"><div className="player-name-card"><div className="eyebrow">PLAYER</div><h2>플레이어의 이름을 입력해주세요.</h2><p>스토리에서 주인공의 이름으로 표시돼.</p><input autoFocus value={playerName} onChange={e=>setPlayerName(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&playerName.trim())setStarted(true)}} placeholder="이름 입력"/><button className="dark-btn" disabled={!playerName.trim()} onClick={()=>setStarted(true)}>시작하기</button></div></div>:body}</div></div></div>;
}
function SharedPreview({project,loading}) {
  const[sceneId,setSceneId]=useState(null),[index,setIndex]=useState(0),[endingId,setEndingId]=useState(null),[reaction,setReaction]=useState(null),[playerName,setPlayerName]=useState(''),[started,setStarted]=useState(false);
  useEffect(()=>{if(project&&!sceneId)setSceneId(project.scenes[0]?.id||null)},[project,sceneId]);
  if(loading)return <div className="shared-preview-page">불러오는 중…</div>;
  ...
  const char=c=>project.characters.find(x=>x.id===c),imgForShared=pr=>x=>{const c=pr.characters.find(ch=>ch.id===x?.speakerId);return x?.speakerId&&x.position!=='none'?c?.illustrations?.find(i=>Number(i.num)===Number(x.illustrationNum))?.dataUrl||c?.illustrations?.[0]?.dataUrl:null};
  const dialogue=(x,next,bg)=><div className="shared-stage" style={bg?.dataUrl?{backgroundImage:`url(${bg.dataUrl})`,backgroundSize:'cover',backgroundPosition:'center'}:undefined}>{imgForShared(project)(x)&&<img key={`${x.id}-${x.bounce?'b':''}`} className={`preview-character-img ${x.position} ${x.bounce?'bounce':''}`} src={imgForShared(project)(x)} alt=""/>}<div className="dialogue-box"><div className="preview-speaker">{display(char(x.speakerId)?.name||'')}</div><div className="preview-text"><TypewriterText key={`${x.id}-${x.text}-${playerName}`} text={display(x.text||' ')}/></div><button className="next-arrow" onClick={next}>›</button></div></div>;
  if(!started)return <div className="shared-preview-page"><div className="shared-preview-head"><b>{project.name}</b><span>미리보기 전용</span></div><div className="player-name-screen shared-name-screen"><div className="player-name-card"><div className="eyebrow">PLAYER</div><h2>플레이어의 이름을 입력해주세요.</h2><p>스토리에서 주인공의 이름으로 표시돼.</p><input autoFocus value={playerName} onChange={e=>setPlayerName(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&playerName.trim())setStarted(true)}} placeholder="이름 입력"/><button className="dark-btn" disabled={!playerName.trim()} onClick={()=>setStarted(true)}>시작하기</button></div></div></div>;
  if(reaction)return <div className="shared-preview-page"><div className="shared-preview-head"><b>{project.name}</b><span>미리보기</span></div>{dialogue({...reaction,text:display(reaction.text)},()=>go(reaction.targetId),scene?getBackgroundAt(scene.blocks,ci,project):null)}</div>;
  if(!sceneId){const e=project.endings.find(x=>x.id===endingId),ei=nextContentIndex(e?.blocks||[],index),eb=e?.blocks?.[ei],bg=e?getBackgroundAt(e.blocks,ei,project):null;return <div className="shared-preview-page"><div className="shared-preview-head"><b>{project.name}</b><span>미리보기 전용</span></div>{eb?dialogue(eb,()=>ei+1<e.blocks.length?setIndex(ei+1):setIndex(-1),bg):<div className="preview-ending-screen"><div className="preview-ending-kicker">ENDING</div><h2>{display(e?.name||'END')}</h2><button className="dark-btn" onClick={()=>{setSceneId(project.scenes[0]?.id||null);setIndex(0);setEndingId(null)}}>처음부터</button></div>}</div>}
  return <div className="shared-preview-page"><div className="shared-preview-head"><b>{project.name}</b><span>미리보기 전용</span></div>{b?.type==='dialogue'?dialogue(b,()=>setIndex(ci+1<scene.blocks.length?ci+1:0),getBackgroundAt(scene.blocks,ci,project)):b?.type==='map'?<MapPreview map={project.maps?.find(m=>m.id===b.mapId)} project={project} playerName={playerName} illustrationFor={imgForShared(project)} onComplete={()=>setIndex(ci+1<scene.blocks.length?ci+1:0)}/>:b?.type==='choice'?<div className="choice-preview"><div className="choice-preview-prompt">{display(b.prompt||'선택하세요.')}</div>{(b.options||[]).map(o=><button className="preview-choice" key={o.id} onClick={()=>{if(b.reactive&&o.responseText?.trim())setReaction({speakerId:o.responseSpeakerId,text:o.responseText,targetId:b.reactiveTargetId});else go(b.reactive?b.reactiveTargetId:o.targetId)}}>{display(o.text||'선택지')}</button>)}</div>:<div className="preview-empty">더 진행할 내용이 없어.</div>}</div>;
}
export default App;
