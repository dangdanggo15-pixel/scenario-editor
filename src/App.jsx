...
function AuthBox({ user, onUserChange, onRefresh }) {
  ...
  const signIn = async mode => {
    if (!supabaseEnabled) return; if (!email.trim() || !password) return setMessage('이메일과 비밀번호를 입력해줘.');
  ...
  const google = async () => { setBusy(true); setMessage('Google 로그인으로 이동 중…'); const { error } = await supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: getAuthRedirectUrl() } }); if (error) { setMessage(error.message); setBusy(false); } };
  if (!supabaseEnabled) return <div className="cloud-card muted"><div className="tiny-label">CLOUD SYNC</div><strong>현재는 로컬 모드</strong><p>Supabase 연결 전에도 편집/미리보기/export는 사용할 수 있어.</p></div>;
...
function App() {
  ...
  const [shareBusy, setShareBusy] = useState(false); const [shareUrl, setShareUrl] = useState(''); const [remoteProjects, setRemoteProjects] = useState([]);
  ...
  const saveCloudProject = useCallback(async (p = projectRef.current, silent=false) => {
    if (!p) return false;
    if (!supabase || !user) { dirtyRef.current = false; setStatus('로컬 저장됨'); setLastSavedAt(new Date()); return true; }
    ...
    try {
      ...
      dirtyRef.current = false; setStatus('저장됨'); setLastSavedAt(new Date()); return true;
    } catch (e) { console.error(e); setStatus('저장 실패 · 로컬에는 보관됨'); return false; }
  }, [user]);
  ...
  return <div className="app-shell">
    ...
    <main className="main-area"><header className="topbar"><div className="topbar-left"><span className="crumb">{selectedTab==='script'?currentScene?.name:selectedTab==='characters'?'등장인물':selectedTab==='endings'?'엔딩':'미리보기'}</span><span className="slash">/</span><span className="muted-text">{project.name}</span></div><div className="topbar-actions"><button className="save-btn" onClick={saveNow}>저장</button><button className="share-btn" onClick={createShare} disabled={shareBusy}>{shareBusy?'링크 생성 중…':'🔗 미리보기 링크'}</button><label className="ghost-btn import-btn">JSON 불러오기<input type="file" accept="application/json,.json" hidden onChange={e=>{const f=e.target.files?.[0];e.target.value='';if(!f)return;const r=new FileReader();r.onload=()=>{try{const next=normalizeProject(JSON.parse(r.result).project||JSON.parse(r.result));replaceProject(next);setProjectId(next.id);setSelectedSceneId(next.scenes[0]?.id||null);}catch{alert('JSON을 읽지 못했어.')}};r.readAsText(f)}}/></label><button className="ghost-btn" onClick={()=>downloadText(makeJson(project),`${safeName(project.name)}.json`)}>JSON 저장</button><button className="ghost-btn" onClick={()=>downloadText(makeInk(project),`${safeName(project.name)}.ink`)}>INK 저장</button><button className="dark-btn" onClick={()=>exportUnity(project)}>Unity Export .zip</button><button className="preview-btn" onClick={startPreview}>▶ 미리보기</button></div></header>
      ...
      <div className="content-wrap">
        {selectedTab==='script'&&currentScene&&<section className="editor-card"><div className="editor-header"><div><input className="scene-title-input" value={currentScene.name} onChange={e=>updateScene(currentScene.id,s=>({...s,name:e.target.value}))}/><div className="editor-hint"><kbd>Tab</kbd> → 다음 행 생성 + 이전 캐릭터 자동 상속 · <b>Num.</b>은 일러스트 버전 · <b>Position</b>은 좌/우 표시</div></div><button className="danger-text-btn" onClick={()=>deleteScene(currentScene.id)} disabled={project.scenes.length<=1}>씬 삭제</button></div>
          <ScriptEditor project={project} scene={currentScene} updateBlock={updateBlock} addDialogueAfter={addDialogueAfter} handleDialogueKeyDown={handleDialogueKeyDown} dialogueRefs={dialogueRefs} deleteBlock={deleteBlock} updateScene={updateScene}/>
        </section>}
